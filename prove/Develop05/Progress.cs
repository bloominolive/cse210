...
public class Progress {
    ...
    public void DisplayGoalList()
    {
        Console.WriteLine("The Goals Are:");
            for (int i = 1; i <= _goalDict.Count(); i++){
                Console.Write($"{i}. {IsAchievedDisplay(_goalDict[i].GetGoalIsAcheived())} {_goalDict[i].GetGoalTitle()} ({_goalDict[i].GetGoalDescription()})");
                if(_goalDict[i] is ChecklistGoal checklistGoal)
                {
                    Console.Write($"-- Currently Completed: {checklistGoal.GetTimesCompleted()}/{checklistGoal.GetTimesToComplete()}");
                }
                Console.WriteLine();
        }
        Console.WriteLine();
      ...
      public void Save(string fileName)
    {
        ...
        foreach (KeyValuePair<int, Goal> goal in _goalDict)
        {
            ...
            if (goal.Value._type == GoalTypes.ChecklistGoal){
                ...
                newLine = $"{checklistGoal.GetGoalType()}|{checklistGoal.GetGoalTitle()}|{checklistGoal.GetGoalDescription()}|{checklistGoal.GetPointsForGoal()}|{checklistGoal.GetGoalIsAcheived()}|{checklistGoal.GetBonusPoints()}|{checklistGoal.GetTimesToComplete()}|{checklistGoal.GetTimesCompleted()}";
            }
            else{
                newLine = $"{goal.Value.GetGoalType()}|{goal.Value.GetGoalTitle()}|{goal.Value.GetGoalDescription()}|{goal.Value.GetPointsForGoal()}|{goal.Value.GetGoalIsAcheived()}";
            }          
    ...
    public void Load(string file){
        ...
        if(file == _loadedFileName){
            Console.WriteLine("File has already been loaded this session.");
            return;
        }
        try{
