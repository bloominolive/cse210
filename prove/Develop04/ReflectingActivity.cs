...
public class ReflectingActivity : Activity {
    ...
    public void RunActivity(){
        RefreshLists();
        BeginActivity(DisplayReflectingActivity);
    }

    private void DisplayReflectingActivity()
    {
        var questionTime = Math.Ceiling(_timeDuration/10.0);
        var count = _questions.Count();
        if (count < questionTime){
            questionTime = count;
            _timeDuration = count * 10;
            Console.WriteLine($"Time was too long. Shortened to {_timeDuration} Seconds ");
        }
